...
import { deleteMeeting, resendMeetingInvite, patchMeeting } from "../../store/actions/meetingsActions";
...
export default function ItemsList(props) {
  ...
  const onDialogSuccess = async (values) => {
    await dispatch(patchMeeting(dialogState.initial.id, values));
    setDialogState({ open: false, initial: null });
  }
  return (
    <>
      {dialogState.open && <CreateEditMeetingDialog open={dialogState.open} initial={dialogState.initial} closeHandler={dialogCloseHandler} successHandler={onDialogSuccess} />}
      <List sx={{
